...
haplotype_dirs = ['1_hap', '3_hap', '5_hap', '10_hap']
depths = ['5x', '10x', '50x', '100x', '200x']

for hap_dir in haplotype_dirs:
    hap_count = int(hap_dir.split('_')[0])
    for depth in depths:
        coverage = int(depth[:-1])
        for replicate in range(1, 21):  # 1 to 20 replicates
            rep_dir = f'sars_depths/{hap_dir}/{depth}/rep{replicate}/'
            
            true_abundance_file = f'{rep_dir}panman_outputs/sars_{hap_count}_abundance.tsv'
            est_abundance_file = f'{rep_dir}panmap_outputs/sars_{hap_count}_iss_abundance.tsv'
            
            # Add the true abundance file
            files[hap_count][replicate]['true'] = [true_abundance_file]
            
            # Add the estimate abundance file
            files[hap_count][replicate]['estimates'].append(est_abundance_file)

groups = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
for n_haplotype in (1, 3, 5, 10):

    for replicate in range(1, 21):  # replicates 1 to 20
        true_abundance = {}
        true_abundance_file = files[n_haplotype][replicate]['true'][0]
        # print(f"Reading true abundance from {true_abundance_file}")
        with open(true_abundance_file) as fh:
            for line in fh.readlines():
                fields = line.strip().split()
                if n_haplotype == 1:
                    print(f"Parsed true abundance: {fields}")  # Debug print
                true_abundance[fields[0]] = float(fields[1])

        for out_abundance_file in files[n_haplotype][replicate]['estimates']:
            # coverage = int(out_abundance_file.split('/')[-4].rstrip('x'))
            # print(f"Reading estimate abundance from {out_abundance_file}")
            estimate_abundance = {}
            with open(out_abundance_file) as fh:
                for line in fh.readlines():
                    if line.startswith('@'): continue
                    fields = line.strip().split()
                    # print(f"Parsed estimated abundance: {fields}")  # Debug print
    ...
    for i, abun in enumerate(true_abundances[num_haplotype]):
        ...
        for j, cov in enumerate((5.0, 10.0, 50.0, 100.0, 200.0)):
            ...
            panel.plot(
                ...
                markeredgewidth=0,
                linewidth=0)
            print(f"Quartiles: {quartiles} for xcoor: {xcoor} with colors: {colors[j]}")
            make_box(panel, xcoor, box_width, quartiles, colors[j])

plt.savefig('sim_results_pure_reads_sars_2k_sars_2k.png', dpi=1200)